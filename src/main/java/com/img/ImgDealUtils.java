...
//        newFolder("d:\\down");
        //将远程图片下载到本地
        List<String> downLoadFilePath = Arrays.asList("D:\\tdt\\img_c\\10\\169\\799.png,D:\\tdt\\img_c\\10\\169\\800.png,D:\\tdt\\img_c\\10\\169\\801.png".split(","));
        //调用方法生成图片>>>>>>>>>>>>>>
        //水平拼接
//        newFolder("D://test");
        ImgDealUtils.merge(downLoadFilePath, "D://tdt//merge.png", true);
        //竖直拼接
//        ImgDealUtils.merge(downLoadFilePath, "D://tdt//merge.png", false);
        //删除某文件夹下全部文件
//        delAllFile("d:\\down");
    }
...
//                String filePath = "d:\\down\\" + fileName;
    ...
    private static BufferedImage buildVerticalImage(List<Integer> widthList, List<Integer> heightList, List<MergeBean> imgInfoList) {
        ...
        BufferedImage imageResult = new BufferedImage(maxWidth, sumHeight, BufferedImage.TYPE_INT_ARGB);
    ...
    private static BufferedImage buildHorizontalImage(List<Integer> widthList, List<Integer> heightList, List<MergeBean> imgInfoList) {
        ...
        // 生成新图片  (新图片的宽度之和，新图片的高度,BufferedImage.TYPE_INT_RGB)
        BufferedImage imageResult = new BufferedImage(sumWidth, maxHeight, BufferedImage.TYPE_INT_ARGB);
        for (int i = 0; i < imgInfoList.size(); i++) {
            if (i == 0) { //第一张图片
                //创建第一张图片
